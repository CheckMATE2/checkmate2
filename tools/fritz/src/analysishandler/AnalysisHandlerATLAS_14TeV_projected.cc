/*
 * AnalysisHandlerATLAS_14TeV_projected.cc
 *
...
const double AnalysisHandlerATLAS_14TeV_projected::DR_TAU_TRACK = 0.2;
const double AnalysisHandlerATLAS_14TeV_projected::PTMIN_TAU_TRACK = 1.0;

const double AnalysisHandlerATLAS_14TeV_projected::DR_TAU_TRUTH = 0.2;
const double AnalysisHandlerATLAS_14TeV_projected::ETAMAX_TAU_TRUTH = 2.5;
const double AnalysisHandlerATLAS_14TeV_projected::PTMIN_TAU_TRUTH = 10.0;

const double AnalysisHandlerATLAS_14TeV_projected::DR_B_TRUTH = 0.4;
const double AnalysisHandlerATLAS_14TeV_projected::ETAMAX_B_TRUTH = 2.5;
const double AnalysisHandlerATLAS_14TeV_projected::PTMIN_B_TRUTH = 1.0;

AnalysisHandlerATLAS_14TeV_projected::AnalysisHandlerATLAS_14TeV_projected() : AnalysisHandler() {
}

AnalysisHandlerATLAS_14TeV_projected::~AnalysisHandlerATLAS_14TeV_projected() {
}

void AnalysisHandlerATLAS_14TeV_projected::initialize() {
}

void AnalysisHandlerATLAS_14TeV_projected::finalize() {
}

void AnalysisHandlerATLAS_14TeV_projected::bookAnalysis(std::string analysisName,
                                        Tag_Map whichTags,
                                        Param_Map eventParameters) {
    Global::print(name, "Loading Analysis "+analysisName);

    AnalysisBase* a = NULL;
    if(analysisName == "atlas_2014_010_hl_3l")
        a = new Atlas_2014_010_hl_3l();
    else if(analysisName == "atlas_phys_2014_010_300")
        a = new Atlas_phys_2014_010_300();
    else if(analysisName == "atlas_phys_2014_010_sq_hl")
        a = new Atlas_phys_2014_010_sq_hl();
    else if(analysisName == "atlas_phys_pub_2013_011")
...
void AnalysisHandlerATLAS_14TeV_projected::postProcessParticles() {
...
void AnalysisHandlerATLAS_14TeV_projected::identifyPhotons() {
    double pEffMed = 0;
    Photon* cand = NULL;
...
void AnalysisHandlerATLAS_14TeV_projected::identifyElectrons() {
...
void AnalysisHandlerATLAS_14TeV_projected::identifyMuons()  {
...
void AnalysisHandlerATLAS_14TeV_projected::tagBJets() {
   ...
   for(int j = 0; j < jets.size(); j++) {
              ...
              if(true_b[b]->PT > PTMIN_B_TRUTH &&
                fabs(true_b[b]->Eta) < ETAMAX_B_TRUTH &&
                 true_b[b]->P4().DeltaR(cand->P4()) < DR_B_TRUTH) {
                  eff_function = &AnalysisHandlerATLAS_14TeV_projected::bSigEff;
                  break;
              }
          }
              ...
              for(int c = 0; c < true_c.size(); c++) {
                  if(true_c[c]->PT > PTMIN_B_TRUTH &&
                     ...
                     true_c[c]->P4().DeltaR(cand->P4()) < DR_B_TRUTH) {
                      eff_function = &AnalysisHandlerATLAS_14TeV_projected::bBkgCJetEff;
                      break;
                  }
              }
          }
          ...
          if (eff_function == NULL)
              eff_function = &AnalysisHandlerATLAS_14TeV_projected::bBkgLJetEff;
...
void AnalysisHandlerATLAS_14TeV_projected::tagTauJets() {
    ...
    for(int j = 0; j < jets.size(); j++) {
       ...
       for(int t = 0; t < true_tau.size(); t++) {
           if(true_tau[t]->PT > PTMIN_TAU_TRUTH &&
              ...
              cand->P4().DeltaR(true_tau[t]->P4()) < DR_TAU_TRUTH) {
               if(prongs > 1) {
                   effFunLoose = &AnalysisHandlerATLAS_14TeV_projected::tauSigEffMultiLoose;
                   effFunMedium = &AnalysisHandlerATLAS_14TeV_projected::tauSigEffMultiMedium;
                   effFunTight = &AnalysisHandlerATLAS_14TeV_projected::tauSigEffMultiTight;
               }
               else {
                   effFunLoose = &AnalysisHandlerATLAS_14TeV_projected::tauSigEffSingleLoose;
                   effFunMedium = &AnalysisHandlerATLAS_14TeV_projected::tauSigEffSingleMedium;
                   effFunTight = &AnalysisHandlerATLAS_14TeV_projected::tauSigEffSingleTight;
               }
               break;
           }
       }
       ...
       if(effFunLoose == NULL) {
           if(prongs > 1) {
               effFunLoose = &AnalysisHandlerATLAS_14TeV_projected::tauBkgEffMultiLoose;
               effFunMedium = &AnalysisHandlerATLAS_14TeV_projected::tauBkgEffMultiMedium;
               effFunTight = &AnalysisHandlerATLAS_14TeV_projected::tauBkgEffMultiTight;
           }
           else {
               effFunLoose = &AnalysisHandlerATLAS_14TeV_projected::tauBkgEffSingleLoose;
               effFunMedium = &AnalysisHandlerATLAS_14TeV_projected::tauBkgEffSingleMedium;
               effFunTight = &AnalysisHandlerATLAS_14TeV_projected::tauBkgEffSingleTight;
           }
       }
       // Now that we know the right function to use, lets tag
...
void AnalysisHandlerATLAS_14TeV_projected::linkObjects() {
...
double AnalysisHandlerATLAS_14TeV_projected::photonEffMedium(double pt,
                                             double eta) {
// https://cds.cern.ch/record/1527529/files/ATL-PHYS-PUB-2013-004.pdf  
// not tested  
    return  (0.83 - 0.26*exp(1. - pt/24.8) );
}


double AnalysisHandlerATLAS_14TeV_projected::electronRecEff(double pt,
                                            double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::electronIDEffLoose(double pt,
                                               double eta) {
    // https://cds.cern.ch/record/1527529/files/ATL-PHYS-PUB-2013-004.pdf
    // loose + Delphes correction
    double id_eff_loose = 1. - 0.103*exp(1. - pt/15.);			   
    return id_eff_loose;
}

double AnalysisHandlerATLAS_14TeV_projected::electronIDEffMedium(double pt,
                                               double eta) {
    // https://cds.cern.ch/record/1527529/files/ATL-PHYS-PUB-2013-004.pdf
    // !!!! medium not defined !!!!!!!!		   
    return 1.;
...
double AnalysisHandlerATLAS_14TeV_projected::electronIDEffTightOverMedium(double pt,
                                                        double eta) {

    // https://cds.cern.ch/record/1527529/files/ATL-PHYS-PUB-2013-004.pdf
    double id_eff_loose = 0.97 - 0.103*exp(1. - pt/15.);		
    double id_eff_tight = 0.85 - 0.191*exp(1. - pt/20.);			     
    return id_eff_tight/id_eff_loose;
}


double AnalysisHandlerATLAS_14TeV_projected::muonEffCombOverCombPlus(double phi,
                                                     double eta) {
    // https://atlas.web.cern.ch/Atlas/GROUPS/PHYSICS/PAPERS/PERF-2015-10/fig_03b.png
    // tuned to MC
    return ( (fabs(eta)<0.1)*0.57 + (fabs(eta)>=0.1)*(fabs(eta)<2.5)*0.99 );
}

double AnalysisHandlerATLAS_14TeV_projected::muonEffCombPlus(double phi,
                                             double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::tauSigEffSingleLoose(double pt,
                                                  double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::tauSigEffSingleMedium(double pt,
                                                   double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::tauSigEffSingleTight(double pt,
                                                  double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::tauSigEffMultiLoose(double pt,
                                                 double eta) {
    const double x0 = 15;
...
double AnalysisHandlerATLAS_14TeV_projected::tauSigEffMultiMedium(double pt,
                                                  double eta) {
    const double x0 = 15;
...
double AnalysisHandlerATLAS_14TeV_projected::tauSigEffMultiTight(double pt,
                                                 double eta) {
    const double x0 = 15;
...
double AnalysisHandlerATLAS_14TeV_projected::tauBkgEffSingleLoose(double pt,
                                                  double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::tauBkgEffSingleMedium(double pt,
                                                   double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::tauBkgEffSingleTight(double pt,
                                                  double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::tauBkgEffMultiLoose(double pt,
                                                 double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::tauBkgEffMultiMedium(double pt,
                                                  double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::tauBkgEffMultiTight(double pt,
                                                 double eta) {
...
double AnalysisHandlerATLAS_14TeV_projected::bSigEff(double pt,
                                     double eta,
                                     double eff) {
...
double AnalysisHandlerATLAS_14TeV_projected::bBkgCJetRej(double eff) {
...
double AnalysisHandlerATLAS_14TeV_projected::bBkgCJetEff(double pt,
                                         double eta,
                                         double eff) {
...
double AnalysisHandlerATLAS_14TeV_projected::bBkgLJetRej(double eff) {
...
double AnalysisHandlerATLAS_14TeV_projected::bBkgLJetEff(double pt,
                                         double eta,
                                         double eff) {
...
const AnalysisHandlerATLAS_14TeV_projected::muonDetector AnalysisHandlerATLAS_14TeV_projected::detMap[53][21] = {{cscla, endla, endla, endla, endla, endla, endla, trans, barla, barla, barla, barla, barla, trans, endla, endla, endla, endla, endla, endla, cscla},
        ...
        {cscsm, endsm, endsm, endsm, endsm, endsm, endsm, trans, barla, barla, barla, barla, barla, trans, endsm, endsm, endsm, endsm, endsm, endsm, cscsm},
        {cscla, endla, endla, endla, endla, endla, endla, trans, barla, barla, barla, barla, barla, trans, endla, endla, endla, endla, endla, endla, cscla}};
const double AnalysisHandlerATLAS_14TeV_projected::etaProj[21] = {-2.005, -1.955, -1.709, -1.411, -1.238, -1.163, -1.113, -0.965, -0.817, -0.519, 0.519, 0.817, 0.965, 1.113, 1.163, 1.238, 1.411, 1.709, 1.955, 2.005, 2.500};
const double AnalysisHandlerATLAS_14TeV_projected::phiProj[53] = {3.142, 3.009, 2.948, 2.849, 2.629, 2.529, 2.471, 2.210, 2.174, 2.065, 1.855, 1.745, 1.695, 1.436, 1.386, 1.276, 1.066, 0.957, 0.916, 0.657, 0.596, 0.496, 0.277, 0.178, 0.142, -0.142, -0.178, -0.277, -0.496, -0.596, -0.657, -0.916, -0.957, -1.018, -1.066, -1.276, -1.357, -1.386, -1.436, -1.695, -1.745, -1.797, -1.855, -2.065, -2.134, -2.174, -2.210, -2.471, -2.529, -2.629, -2.849, -2.948, -3.009};
