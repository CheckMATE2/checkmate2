...
bool SLHABlock::has_entry(std::vector<int> indices)
{
  if (_entries.find(indices) == _entries.end()){
    return false;
  }
...
double SLHAReader::get_block_entry(std::string block_name, std::vector<int> indices, double def_val)
{
...
double SLHAReader::get_block_entry(std::string block_name, int index, double def_val)
{
  std::vector<int> indices;
...
void SLHAReader::set_block_entry(std::string block_name, int index, double value)
{
  std::vector<int> indices;
  indices.push_back(index);
...
bool SLHAReader::has_block_entry(std::string block_name, std::vector<int> indices)
{
  if (_blocks.find(block_name) == _blocks.end()){
    return false;
  }
  SLHABlock block = _blocks[block_name];
  if (!block.has_entry(indices)){
    return false;
  }
  return true;
}
...
bool SLHAReader::has_block_entry(std::string block_name, int index)
{
  std::vector<int> indices;
  indices.push_back(index);
  return has_block_entry(block_name, indices);
}