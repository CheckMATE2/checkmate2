...
class SLHABlock
{
  public:
    ...
    bool has_entry(std::vector<int> index);
...
class SLHAReader
{
  public:
    ...
    void set_block_entry(std::string block_name, int index, 
			   double value);
    bool has_block_entry(std::string block_name, std::vector<int> index);
    bool has_block_entry(std::string block_name, int index);
