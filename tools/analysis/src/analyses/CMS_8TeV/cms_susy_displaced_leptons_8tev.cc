...
void Cms_susy_displaced_leptons_8tev::analyze() {
  ...
  if (evt_weight > r1) return;

  countCutflowEvent("Cut 7: Combined efficiency cut");

  if (fabs(el[0]->D0) > 1. && fabs(mu[0]->D0) > 1.) countSignalEvent("SR3");
  else if (fabs(el[0]->D0) > 0.5 && fabs(mu[0]->D0) > 0.5) countSignalEvent("SR2");
  else if (fabs(el[0]->D0) > 0.2 && fabs(mu[0]->D0) > 0.2) countSignalEvent("SR1");
