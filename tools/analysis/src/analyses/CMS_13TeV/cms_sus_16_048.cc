...
// AUTHOR:Junjie Cao, Liangliang Shang, Jin Min Yang, Yuanfang Yue and Yang Zhang 
// EMAIL: junjiec@itp.ac.cn, shlwell1988@gmail.com, jmyang@itp.ac.cn,yuanfang405@gmail.com and zhangyang@itp.ac.cn 

void Cms_sus_16_048::initialize() {
  setAnalysisName("cms_sus_16_048");          
  setInformation(""
		 "# cms_sus_16_048\n"
		 "# two soft leptons, jets, missingET\n"
		 "# sqrt(s)=13TeV\n"
		 "# int(L)=35.9 fb^-1 and 33.2 fb^-1\n"
...
//electrons could be used but with worse case compared to electronsLoose and electronsMedium 
  ...
  muons_g = filterPhaseSpace(muons, 3, -2.4, 2.4);
  ...
  countCutflowEvent("CR01_2SOLep");

  //for lepton's pt and eta
  for (int i=0; i<leptons.size(); i++) {
    if(leptons[i]->PT < 3.5) return;
    if(leptons[i]->PT > 30) return;
    if(leptons[i]->Type == "muon") {
      if (fabs(leptons[i]->Eta) > 2.4) return;
    } else {
      if (fabs(leptons[i]->Eta) > 2.5) return;
    }
  }
  ...
  bool lep_good = true;
  for (int i=0; i<leptons.size(); i++) {
    if (leptons[i]->PT < 5.) lep_good=false;
  }

  //classification of flavor of leptons
  bool SFflag = false;
  bool mmflag = false;
  if( leptons[0]->Type == leptons[1]->Type ) {
    SFflag = true;
    if( leptons[0]->Type == "muon" ) mmflag = true;
  }
  ...
  if ( mT(leptons[0]->P4(), missingET->P4()) < 70. ) {
    if ( mT(leptons[1]->P4(), missingET->P4()) < 70. ) {
      mt_good = true;
    }
  }

  //for Signal regions of electroweakino case
  if(lep_good and SFflag and mt_good) {

    countCutflowEvent("CR13_mT_weakino");

    if(mET<200.) {
      if ( not mmflag ) return;
      fout1 << mmflag << std::endl;
      if (mll < 10.) {
      ...
      if (mll < 10.) {
        countCutflowEvent("SR1_weakino_2media_mll_1");
        countSignalEvent("SR1_weakino_2media_mll_1");
        
      } else if (mll < 20.) {
        countCutflowEvent("SR1_weakino_2media_mll_2");
        countSignalEvent("SR1_weakino_2media_mll_2");
      } else if (mll < 30.) {
        countCutflowEvent("SR1_weakino_2media_mll_3");
        countSignalEvent("SR1_weakino_2media_mll_3");
      } else {
        countCutflowEvent("SR1_weakino_2media_mll_4");
      ...
      if (mll < 10.) {
        ...
        countCutflowEvent("SR1_weakino_3high_mll_2");
        countSignalEvent("SR1_weakino_3high_mll_2");
      } else if (mll < 30.) {
        countCutflowEvent("SR1_weakino_3high_mll_3");
        countSignalEvent("SR1_weakino_3high_mll_3");
      } else {
        countCutflowEvent("SR1_weakino_3high_mll_4");
        countSignalEvent("SR1_weakino_3high_mll_4");
      }
    }
  }
  ...
  if(mET<200.) {
    ...
    if ( not lep_good ) return;
    if (leptons[0]->PT < 12.) {
      countCutflowEvent("SR2_stop_2media_pt_1");
      countSignalEvent("SR2_stop_2media_pt_1");
    } else if (leptons[0]->PT < 20.) {
      countCutflowEvent("SR2_stop_2media_pt_2");
      countSignalEvent("SR2_stop_2media_pt_2");
    } else {
      countCutflowEvent("SR2_stop_2media_pt_3");
      countSignalEvent("SR2_stop_2media_pt_3");
    }
  } else {
    if (leptons[0]->PT < 12.) {
      countCutflowEvent("SR2_stop_3high_pt_1");
      countSignalEvent("SR2_stop_3high_pt_1");
    } else if (leptons[0]->PT < 20.) {
      countCutflowEvent("SR2_stop_3high_pt_2");
      countSignalEvent("SR2_stop_3high_pt_2");
    } else {
      countCutflowEvent("SR2_stop_3high_pt_3");
      countSignalEvent("SR2_stop_3high_pt_3");
    }
  }
