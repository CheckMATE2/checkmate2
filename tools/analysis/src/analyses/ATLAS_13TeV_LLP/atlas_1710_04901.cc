...
//follow https://atlas.web.cern.ch/Atlas/GROUPS/PHYSICS/PAPERS/SUSY-2016-08/hepdata_info.pdf
// AUTHOR: Florian Domingo and Zeren Simon Wang
//  EMAIL: domingo@physik.uni-bonn.de      wzs@mx.nthu.edu.tw
...
//  countCutflowEvent("1_metcut");
  ...
  for(int i=0;i<true_particles.size();++i){               // loop over particles in the decay table
   ...
   if(abs(part->PT) > 1. && fabs(apimppar) > 2.){           // check if the particle qualifies as a selected decay product
